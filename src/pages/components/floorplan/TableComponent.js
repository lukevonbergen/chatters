...
const TableComponent = ({ table, editMode, onRemoveTable, onTableResize }) => {
  ...
  // Disable text selection while dragging (nice UX)
  useEffect(() => {
  ...
  // Store the element that captured the pointer + its id to avoid null currentTarget
  ...
  const startResize = useCallback((e, direction, circle = false) => {
    ...
    // Remember capture target and pointer id for a clean release later
    ...
    const cleanUp = () => {
      // Safely release capture on the original element
    ...
    >
      ...
      {editMode && (
        <>
          ...
          {/* Resize handles - non-circle shapes */}
