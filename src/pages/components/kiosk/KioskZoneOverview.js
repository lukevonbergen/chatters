...
const KioskZoneOverview = ({ zones, tables, feedbackMap, feedbackList, onZoneSelect }) => {
  ...
  const getTableShapeClasses = (shape, feedbackStatus) => {
    ...
    switch (shape) {
      case 'circle':
        return { className: `${baseClasses} w-12 h-12 rounded-full hover:bg-gray-600`, style: pulseStyle };
      case 'long':
        return { className: `${baseClasses} w-20 h-8 rounded-lg hover:bg-gray-600 text-sm`, style: pulseStyle };
      default:
        return { className: `${baseClasses} w-12 h-12 rounded-lg hover:bg-gray-600`, style: pulseStyle };
    }
  };
  ...
  const renderTable = (table) => {
    ...
    return (
      ...
      >
        <div className={tableShapeConfig.className} style={tableShapeConfig.style}>
          {table.table_number}
        </div>
      </div>
    );
        ...
        {zones.length === 0 ? (
          ...
          <div className="space-y-8">
            {zones.map(zone => {
              ...
              const zoneAlerts = feedbackList.filter(f => zoneTables.some(t => t.table_number === f.table_number));
              ...
              return (
                <div key={zone.id} className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                  ...
                  {zoneTables.length === 0 ? (
                    ...
                    <div className="grid grid-cols-10 md:grid-cols-14 lg:grid-cols-18 gap-2">
